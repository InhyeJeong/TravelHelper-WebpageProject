...
import edu.iot.common.model.Review;
import edu.iot.common.model.ReviewImage;
import edu.iot.common.service.ReviewService;
...
public class ReviewController {
	...
	public void list(
					...
					@RequestParam(value="orderBy", defaultValue="review_id") String orderBy,
	...
	public void createForm(Review review)throws Exception {
		
	}
	
	@RequestMapping(value="create", method=RequestMethod.POST)
	public String createSubmit(@Valid Review review,
		...
		log.info(review.toString());//galleryId가 지정되어 있지 않다 seq를 통해 지정된다.

		return "redirect:/review/update/" + review.getReviewId();
	...
	/** update */
	@RequestMapping(value="/update/{reviewId}", method=RequestMethod.GET)
	public String updateForm(
			@PathVariable long reviewId, Model model, ReviewImage image) throws Exception {
	 ...
	 * 이미 review에 reviewId가 폼태그에 의해 저장되어 있으므로 따로 @PathVarialble 을 사용할 필요가 없다.
	 * 위의 updatForm에는 어노테이션을 사용하여 변수로 받아줘야한다.*/
	@RequestMapping(value="/update/{reviewId}", method=RequestMethod.POST)
	public String updateSubmit(@Valid Review review,
								BindingResult result) throws Exception {
	...
	public String addImageSubmit(ReviewImage image,
		...
		//db저장
		return "redirect:/review/update/" + image.getReviewId();
		
	}
	
	/**단일 파일 업로드 처리시 사용 // MultipartFile 사용*/
	
	private void uploadImage(ReviewImage image, MultipartFile imageFile) throws Exception {
		//원본 이미지 파일명
	...
	public String addImagesSubmit(ReviewImage image,
	...
	public String image (
		...
		ReviewImage image = service.getImage(imageId);
